...
const TimeSlotModal = ({ 
  ...
  const testBackendConnection = async () => {
    try {
      ...
      const response = await axios.get(`${API_BASE_URL}/`, { timeout: 5001 });
      setConnectionTest({
        success: true,
  ...
  const verifyPersonExists = async () => {
    ...
    try {
      ...
      try {
        const directEndpoint = `${API_BASE_URL}/${personType}s/${scannedPerson.id}`;
  ...
  const handleSetTimer = async () => {
    ...
    try {
      ...
      const timerEndpoint = `${API_BASE_URL}/${scannedPerson.personType}s/${scannedPerson.id}/set-custom-timer`;
          ...
          <>
            {scannedPerson?.personType === 'guest' && (
              <Alert variant="secondary" className="py-2 mb-3">
                <Form.Check
                  type="switch"
                  id="guest-open-ended-switch-admin"
