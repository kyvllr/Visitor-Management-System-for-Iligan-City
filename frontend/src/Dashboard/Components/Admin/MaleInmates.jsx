...
const MaleInmates = () => {
  ...
  const searchOptions = [
    ...
    { value: 'inmateCode', label: 'PDL Code' },
  ...
  const handleView = async (inmate) => {
    ...
    // Fetch visitors for this PDL using the new endpoint
  ...
  const handleDelete = async (inmateCode) => {
    if (!window.confirm('Are you sure you want to delete this PDL?')) {
      return;
    }
    ...
    try {
      ...
      toast.success('PDL deleted successfully!');
      fetchInmates();
    } catch (error) {
      console.error('Error deleting PDL:', error);
      toast.error('Failed to delete PDL');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.log("📤 Submitting male PDL data...");
  ...
  printWindow.document.write(`
    <html>
      <head>
        <title>Male PDL Details - ${selectedInmate?.inmateCode}</title>
        <style>
          body { 
      ...
      <body>
        <div class="header">
          <h1>ILIGAN CITY JAIL - MALE DIVISION</h1>
          <h2>Region 10</h2>
          <h3>MALE PDL DETAILS RECORD - ID: ${selectedInmate?.inmateCode}</h3>
        </div>
        ...
        ${selectedInmate ? `
          ...
          <div class="section">
            <div style="text-align: center; margin-top: 30px; padding-top: 20px; border-top: 1px solid #ddd;">
              ...
              <p><em>Official Document - ILIGAN CITY JAIL Male Division, Region 10</em></p>
            </div>
          </div>
        ` : ''}
      </body>
...
>
        <Modal.Header closeButton>
          <Modal.Title>{editingInmate ? 'Edit Male PDL' : 'Add New Male PDL'}</Modal.Title>
        </Modal.Header>
        <Form onSubmit={handleSubmit}>
          ...
          <Modal.Footer className="py-2">
            ...
            <Button variant="dark" type="submit" disabled={isLoading}>
              {isLoading ? <Spinner size="sm" /> : (editingInmate ? 'Update' : 'Add') + ' Male PDL'}
            </Button>
          </Modal.Footer>
        </Form>
      </Modal>
        ...
        <Modal.Body>
          <Alert variant="info" className="mb-4">
            ...
            <ul className="mb-0 mt-2">
              ...
              <li><strong>Gender</strong> (required - Must be "Male")</li>
