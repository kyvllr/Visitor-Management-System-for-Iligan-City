...
const VisitorMaleDivision = () => {
  ...
  useEffect(() => {
    filterVisitors();
  }, [searchQuery, searchBy, categoryFilter, visitors, maleInmates]);
  ...
  const getMaleDivisionVisitors = () => {
    if (maleInmates.length === 0) return [];
    const maleInmateCodes = new Set(maleInmates.map(inmate => inmate.inmateCode));
    return visitors.filter(visitor => maleInmateCodes.has(visitor.prisonerId));
  };

  const printAllVisitorQRCodes = () => {
    const maleDivisionVisitors = getMaleDivisionVisitors();
    printVisitorQRCards({
      visitors: maleDivisionVisitors,
      apiBaseUrl: API_BASE_URL,
      cardTitle: "ICJ-MD VISITOR'S ID SYSTEM",
      documentTitle: 'Male Division Visitor QR IDs',
      styleConfigOverrides: STANDARD_VISITOR_QR_CARD_OVERRIDES,
      getPdlName: (visitor) => {
        const matchedInmate = maleInmates.find(inmate => inmate.inmateCode === visitor.prisonerId);
        ...
        <Form onSubmit={handleSubmit}>
          <Modal.Body style={{ maxHeight: '70vh', overflowY: 'auto' }}>
            <Alert variant="info" className="mb-3">
              <strong>Male Division:</strong> Visitors can only be connected to male PDL.
            </Alert>
              ...
              <Col md={6}>
                <Form.Group className="mb-3">
                  ...
                  <Form.Text className="text-muted">
                    Only male PDL available for selection
                  </Form.Text>
            ...
            <Alert variant="info" className="mt-3">
              ...
              Visitors can only be connected to male PDL in this division.
            </Alert>
          </Modal.Body>
