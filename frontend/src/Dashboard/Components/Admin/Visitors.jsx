...
const Visitors = () => {
  ...
  const [visitorLimit, setVisitorLimit] = useState(null);
  ...
  const selectPrisonerSuggestion = async (inmate) => {
    ...
    // Check visitor limit for this inmate
    await checkVisitorLimit(inmate.inmateCode);
  };

  const selectPrisonerNameSuggestion = async (inmate) => {
    setPrisonerNameInput(inmate.fullName);
    setPrisonerIdInput(inmate.inmateCode);
    setPrisonerNameSuggestions([]);
    setPrisonerIdSuggestions([]);
  ...
  const checkVisitorLimit = async (inmateCode) => {
    if (!inmateCode) {
      setVisitorLimit(null);
      return;
    }
  ...
  const handleEdit = (visitor) => {
    ...
    // Check visitor limit for the current PDL
    if (visitor.prisonerId) {
      checkVisitorLimit(visitor.prisonerId);
    }
  ...
  const handleSubmit = async (e) => {
    ...
    // Check visitor limit before submitting (only for new visitors, not edits)
    if (!editingVisitor && visitorLimit && !visitorLimit.canAddMore) {
      toast.error(`Cannot add visitor. This PDL has reached the maximum limit of ${visitorLimit.limit} visitors.`);
      setIsLoading(false);
      return;
    }
    ...
    try {
      ...
      // Format data properly - backend will handle prisoner name lookup
      const formattedData = {
        ...formData,
      ...
      <Card style={{ 
        ...
        <Card.Body>
          <Row className="align-items-center">
            ...
            <Col md={4}>
              <div style={{ color: '#ffffff', fontWeight: '500', textAlign: 'right' }}>
...
>
        <Modal.Header closeButton>
          <Modal.Title>{editingVisitor ? 'Edit Visitor' : 'Add New Visitor'}</Modal.Title>
        </Modal.Header>
        <Form onSubmit={handleSubmit}>
          <Modal.Body style={{ maxHeight: '70vh', overflowY: 'auto' }}>
            ...
            {visitorLimit && (
              <Alert 
                variant={visitorLimit.canAddMore ? (visitorLimit.available <= 2 ? 'warning' : 'info') : 'danger'}
                className="mb-3"
              >
                <div className="d-flex justify-content-between align-items-center">
                  <div>
                    <strong>Visitor Limit Status:</strong> {visitorLimit.currentVisitors} of {visitorLimit.limit} visitors assigned
                  </div>
                  <Badge bg={visitorLimit.canAddMore ? 'success' : 'danger'}>
                    {visitorLimit.available} slots available
                  </Badge>
                </div>
                {!visitorLimit.canAddMore && (
                  <div className="mt-2">
                    <small>⚠️ This PDL has reached the maximum limit of {visitorLimit.limit} visitors. Cannot add more visitors.</small>
                  </div>
                )}
                {visitorLimit.canAddMore && visitorLimit.available <= 2 && (
                  <div className="mt-2">
                    <small>⚠️ Only {visitorLimit.available} visitor slot(s) remaining for this PDL.</small>
                  </div>
                )}
              </Alert>
            )}
