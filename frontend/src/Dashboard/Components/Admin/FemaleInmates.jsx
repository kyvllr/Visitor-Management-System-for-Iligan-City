          ...
          <Row className="mt-2">
            <Col>
              <div className="text-muted small">
                Showing {filteredInmates.length} female PDL
              </div>
            </Col>
          </Row>
        </Card.Body>
      ...
      {isLoading && inmates.length === 0 ? (
        <div className="text-center">
          <Spinner animation="border" role="status">
            <span className="visually-hidden">Loading female PDL...</span>
          </Spinner>
        </div>
      ) : filteredInmates.length === 0 ? (
        <Alert variant="info">
          {searchQuery
            ? 'No female PDL found matching your criteria.' 
            : 'No female PDL found. Add your first female PDL to get started.'}
        </Alert>
      ) : (
