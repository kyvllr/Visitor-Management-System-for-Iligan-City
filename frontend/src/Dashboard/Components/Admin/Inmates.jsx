...
const Inmates = ({ gender = 'all' }) => {
  ...
  const initialGender = location.state?.gender || gender;
  ...
  const filterInmates = () => {
    ...
    if (selectedGender !== 'all') {
      filtered = filtered.filter(inmate => 
        inmate.sex?.toLowerCase() === selectedGender.toLowerCase()
      );
    }
  ...
  const handleAdd = () => {
    ...
    const initialData = {
      ...
      middleName: '',
      extension: '',
      sex: selectedGender !== 'all' ? selectedGender.charAt(0).toUpperCase() + selectedGender.slice(1) : '',
      dateOfBirth: '',
      address: '',
      maritalStatus: '',
      eyeColor: '',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingInmate) {
        ...
        toast.success('PDL created successfully!');
      }
      ...
      console.error('Error submitting PDL:', error);
      console.error('Error response:', error.response?.data);
      
      // Show more detailed error message
      const errorMessage = error.response?.data?.message || 
                          error.response?.data?.error || 
                          `Failed to ${editingInmate ? 'update' : 'create'} PDL`;
  ...
  try {
    ...
    if (response.data.imported > 0) {
      toast.success(`Successfully imported ${response.data.imported} PDL`);
    } else {
      toast.warning(`No PDL imported. ${response.data.errors?.length || 0} errors found.`);
    }
    ...
    setCsvFile(null);
    fetchInmates(); // Refresh the PDL list
  } catch (error) {
  ...
  const exportToCSV = () => {
    ...
    link.href = url;
    link.download = `inmates_${new Date().toISOString().split('T')[0]}.csv`;
    link.click();
    window.URL.revokeObjectURL(url);
    
    toast.success(`Exported ${filteredInmates.length} PDL to CSV`);
  };
  ...
  const getGenderTitle = () => {
    switch (selectedGender) {
      case 'male': return 'Male PDL Management';
      case 'female': return 'Female PDL Management';
      default: return 'All PDL Management';
    }
  };
  ...
  const printInmateDetails = () => {
    ...
    printWindow.document.write(`
      <html>
        <head>
          <title>PDL Details - ${selectedInmate?.inmateCode}</title>
          <style>
            body { 
        ...
        <body>
          <div class="header">
            ...
            <h2>Region 10</h2>
            <h3>PDL DETAILS RECORD - ID: ${selectedInmate?.inmateCode}</h3>
          </div>
  ...
  return (
    <Container>
      ...
      <div className="d-flex justify-content-between align-items-center mb-4">
        <div>
          <h2 className="white-title" style={{ 
          ...
          <Badge bg="info" className="mb-2">
            {selectedGender === 'all' ? 'Full Access' : `${selectedGender.charAt(0).toUpperCase() + selectedGender.slice(1)} Access Only`}
          </Badge>
        </div>
          ...
          <Row className="mt-2">
            <Col>
              <div style={{ color: '#ffffffff', fontWeight: '500', textAlign: 'right' }}>
                Showing {filteredInmates.length} PDL 
                {selectedGender !== 'all' && ` (${selectedGender} only)`}
              </div>
            </Col>
          </Row>
        </Card.Body>
      ...
      {isLoading && inmates.length === 0 ? (
        <div className="text-center">
          <Spinner animation="border" role="status">
            <span className="visually-hidden">Loading PDL...</span>
          </Spinner>
        </div>
      ) : filteredInmates.length === 0 ? (
        <Alert variant="info">
          {searchQuery || selectedGender !== 'all' 
            ? 'No PDL found matching your criteria.' 
            : 'No PDL found. Add your first PDL to get started.'}
        </Alert>
      ) : (
          ...
          <tbody>
            {filteredInmates.map(inmate => (
              <tr key={inmate._id}>
                ...
                <td>
                  <Badge bg={inmate.sex === 'Male' ? 'primary' : 'danger'}>
                    {inmate.sex}
                  </Badge>
                </td>
                <td>{inmate.cellId}</td>
...
>
        <Modal.Header closeButton>
          <Modal.Title>{editingInmate ? 'Edit PDL' : 'Add New PDL'}</Modal.Title>
        </Modal.Header>
        ...
        <Modal.Footer className="py-2">
          ...
          <Button variant="dark" type="submit" disabled={isLoading}>
            {isLoading ? <Spinner size="sm" /> : (editingInmate ? 'Update' : 'Add') + ' PDL'}
          </Button>
        </Modal.Footer>
      </Form>
    </Modal>
...
>
  <Modal.Header closeButton className="py-2">
    <Modal.Title className="fs-5">Import PDL from CSV</Modal.Title>
  </Modal.Header>
  <Modal.Body style={{ maxHeight: "60vh", overflowY: "auto" }}>
