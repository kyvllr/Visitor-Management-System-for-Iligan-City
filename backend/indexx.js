...
const inmateSchema = new mongoose.Schema({
  ...
  leftImage: String,
  rightImage: String,
  isVisitBanned: { type: Boolean, default: false },
  banReason: String,
  banType: String,
  banStartDate: Date,
  banEndDate: Date,
...
app.post("/scan-process", async (req, res) => {
  try {
    ...
    const formatRemainingBanDuration = (endDateValue) => {
      if (!endDateValue) return 'Permanent';

      const endDate = new Date(endDateValue);
      if (Number.isNaN(endDate.getTime())) return 'Unknown duration';
    ...
    let linkedPdlBan = null;
    if (!isGuest && person.prisonerId) {
      const linkedInmate = await Inmate.findOne({ inmateCode: person.prisonerId });

      if (linkedInmate) {
        const banState = getInmateBanState(linkedInmate);
        linkedPdlBan = {
          isBanned: banState.blocked,
          reason: linkedInmate.banReason || 'No reason provided',
          banType: linkedInmate.banType || null,
          banStartDate: linkedInmate.banStartDate || null,
          banEndDate: linkedInmate.banEndDate || null,
          banDuration: linkedInmate.banDuration || null,
          remainingDuration: banState.blocked
            ? formatRemainingBanDuration(linkedInmate.banEndDate)
            : null
        };
      }
    }

    if (!isGuest && linkedPdlBan?.isBanned) {
      const linkedBanMessage = `PDL linked to this visitor is currently not allowed for visitation. Duration: ${linkedPdlBan.remainingDuration || linkedPdlBan.banDuration || 'N/A'}. Reason: ${linkedPdlBan.reason || 'No reason provided'}`;
...
app.put("/visitors/:id/approve-time-in", async (req, res) => {
  try {
    ...
    const linkedInmate = visitor.prisonerId
      ? await Inmate.findOne({ inmateCode: visitor.prisonerId })
      : null;

    if (linkedInmate) {
      const linkedBanState = getInmateBanState(linkedInmate);

      if (linkedBanState.blocked) {
        const remainingDuration = linkedInmate.banEndDate
          ? (() => {
              const diffMs = new Date(linkedInmate.banEndDate).getTime() - Date.now();
              if (Number.isNaN(diffMs)) return linkedInmate.banDuration || 'Unknown duration';
              if (diffMs <= 0) return 'Expired';
        ...
        return res.status(403).json({
          message: `PDL linked to this visitor is currently not allowed for visitation. Duration: ${remainingDuration}. Reason: ${linkedInmate.banReason || 'No reason provided'}`,
          linkedPdlBan: {
            isBanned: true,
            reason: linkedInmate.banReason || 'No reason provided',
            banType: linkedInmate.banType || null,
    ...
    if (visitor?.customTimer?.openEnded) {
      timerStart = new Date();
      timerEnd = null;
      timerDuration = "Open-ended";
      timerMessage = "Open-ended timer active: visitor can time out anytime";
...
const getInmateBanState = (inmate) => {
  if (!inmate?.isVisitBanned) {
    return { blocked: false, expired: false };
  }

  if (!inmate.banEndDate) {
    return { blocked: true, expired: false };
  }
  ...
  return { blocked: !expired, expired };
};
...
app.get("/inmates", async (req, res) => {
  try {
    ...
    const inmatesWithFullName = inmates.map(inmate => ({
      ...
      isVisitBanned: getInmateBanState(inmate).blocked
    }));
...
app.put("/inmates/:code/visit-ban", async (req, res) => {
  try {
    const {
      banReason,
      banType,
    ...
    if (!banReason || !banType || !banStartDate || !banDuration) {
      return res.status(400).json({ message: "Ban reason, type, start date, and duration are required" });
    }

    const startDate = new Date(banStartDate);
    if (Number.isNaN(startDate.getTime())) {
      return res.status(400).json({ message: "Invalid ban start date" });
    }

    let parsedEndDate = null;
    if (banType !== 'permanent') {
      if (!banEndDate) {
        return res.status(400).json({ message: "Ban end date is required for non-permanent bans" });
      }

      parsedEndDate = new Date(banEndDate);
      if (Number.isNaN(parsedEndDate.getTime())) {
        return res.status(400).json({ message: "Invalid ban end date" });
      }

      if (parsedEndDate <= startDate) {
        return res.status(400).json({ message: "Ban end date must be after ban start date" });
      }
    }
    ...
    const inmateWithFullName = {
      ...
      isVisitBanned: getInmateBanState(updatedInmate).blocked
    };

    res.json({ message: "PDL visit ban applied", inmate: inmateWithFullName });
  } catch (error) {
    res.status(500).json({ message: "Failed to apply PDL ban", error: error.message });
  }
});

app.put("/inmates/:code/remove-visit-ban", async (req, res) => {
  try {
    const updatedInmate = await Inmate.findOneAndUpdate(
      { inmateCode: req.params.code },
      {
    ...
    res.json({ message: "PDL visit ban removed", inmate: inmateWithFullName });
  } catch (error) {
    res.status(500).json({ message: "Failed to remove PDL ban", error: error.message });
  }
});
  ...
  async (req, res) => {
    try {
      ...
      if (req.body.prisonerId) {
        ...
        if (inmate) {
          const banState = getInmateBanState(inmate);
          if (banState.blocked) {
            return res.status(403).json({
              message: `PDL is currently blocked from visits. Reason: ${inmate.banReason || 'No reason provided'}`
            });
          }
...
app.get("/visitors/:id", async (req, res) => {
  try {
    ...
    const inmate = await Inmate.findOne({ inmateCode: visitor.prisonerId });
    if (!inmate) {
      return res.status(404).json({ message: "Associated PDL not found" });
    }
...
app.post("/pending-visitors/:id/approve", async (req, res) => {
  try {
    ...
    if (pendingVisitor.prisonerId) {
      const inmate = await Inmate.findOne({ inmateCode: pendingVisitor.prisonerId });
      if (!inmate) {
        return res.status(400).json({ message: `Prisoner with ID ${pendingVisitor.prisonerId} not found` });
      }

      const banState = getInmateBanState(inmate);
      if (banState.blocked) {
        return res.status(403).json({
          message: `Cannot approve visitor. PDL is currently blocked from visits. Reason: ${inmate.banReason || 'No reason provided'}`
        });
      }
...
app.put("/guests/:id/set-custom-timer", async (req, res) => {
  try {
    const { startTime, endTime, duration, openEnded = false } = req.body;
    console.log('🔄 SETTING CUSTOM TIMER FOR GUEST:', req.params.id, { startTime, endTime, duration, openEnded });
    
    if (!openEnded && (!startTime || !endTime)) {
      return res.status(400).json({ message: "Start time and end time are required" });
    }
    ...
    if (openEnded) {
      ...
      return res.json({
        message: 'Open-ended timer set successfully',
        timerStart: timerStart,
        timerEnd: null,
        duration: 'Open-ended',
        durationMinutes: null,
...
app.get("/verify-custom-timer-guest/:id", async (req, res) => {
  try {
    ...
    const hasOpenEndedTimer = !!guest.customTimer?.openEnded;
    const hasCustomTimer = hasOpenEndedTimer || !!(guest.timerStart && guest.timerEnd);
    ...
    res.json({
      hasCustomTimer: hasCustomTimer,
      hasOpenEndedTimer: hasOpenEndedTimer,
      timerStart: guest.timerStart,
      timerEnd: guest.timerEnd,
      customTimer: guest.customTimer,
      message: hasCustomTimer ? 
        (hasOpenEndedTimer
          ? 'Open-ended timer set'
